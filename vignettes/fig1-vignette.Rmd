...
vignette: >
  %\VignetteIndexEntry{fig1-vignette}
...
The purpose of this vignette is to show how to make hospitalization predictions using the COVIDNearTerm method using the COVIDNearTerm library.  We will focus on recreatig a figure like Figure 1 in (ref paper).


```{r eval = FALSE}
install.packages('remotes')
remotes::install_github(repo = 'olshena/COVIDNearTerm')
library(COVIDNearTerm)
```

The county_hospitalizations contains 308 days worth of hospitalization data from six Bay Area counties starting on May 1, 2020. We will make predictions using the 150 days from Santa Clara County as training data.
...
method is how to weigh the wsize prior days of hospitalizations, choices are equal (default), unequal, and traingle
...
```{r}
predicted_paths <- simulateAR(y         = training_data,
                              wsize       = 14,
                              method      = "unweighted",
                              pdays       = 28, 
                              lambda      = seq(0,1,0.05),
...
We plot the data.  The black line is the truth, the green line is predictions for 28 days, the red lines are paths.
