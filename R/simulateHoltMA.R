...
#' @export
#'
simulateHoltMA <- function(dat, pdays, nsim, level = 0.95, bootstrap, ma, smooth_type = "s", model_type = "AAN", seed = NULL){

  if( !is.null(seed) ){
    set.seed(seed)
  }
  n.dat <- length(dat)
  n.dat.pdays <- n.dat+pdays
  paths <- matrix(NA,nsim,n.dat.pdays)
  paths[,1:n.dat] <- matrix(rep(dat,nsim),ncol=n.dat,byrow=TRUE)
  model.ets <- forecast::ets(dat, model = model_type)
  paths[,(n.dat+1):n.dat.pdays] <- modifiedPegelsfcastC(pdays = pdays,
                                                    obj = model.ets,
                                                    nsim = nsim,
                                                    level = level,
  ...
  max_predictions <- apply(paths_ma[,(n.dat+1):n.dat.pdays],1,max)
